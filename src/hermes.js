...
async function analisar(dias = 30) {
  ...
  if (dados.problemas.length >= MINIMO) {
    try {
      // `barato: true` — hoje ele não escolhe mais o modelo (só existe um), e
      // sim o PRAZO: é a declaração de que ninguém está esperando na tela.
      //
      // Este é o trabalho do projeto que mais precisa disso. A entrada é enorme
      // (40 mensagens do outro lado, mais o repertório inteiro) e a saída é
      // longa, então os 25 segundos do atendimento cortariam a análise no meio
      // e jogariam fora o token já gasto. O que sai daqui é um arquivo que uma
      // pessoa lê e decide se aproveita, então esperar um minuto não custa nada
      // a ninguém.
